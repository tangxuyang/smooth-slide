 ...
 */
 ...
 (function (global, factory) {
  ...
  SmoothSlide.prototype.addItem = function(item) {
    ...
    if (this.options.autoPlay) {
      ...
      if (firstItem) {
        ...
        this.content.insertBefore(item, this.emptyItem)
        this.content.style.top = this.content.offsetTop - (this.itemHeight || (item.offsetHeight + this.options.heightDelta)) + 'px'
        this.slideDown(this.itemHeight || (item.offsetHeight + this.options.heightDelta))
        // this.content.appendChild(item)
      }
    }
  ...
  // 向下移动itemHeight
  SmoothSlide.prototype.slideDown = function(height) {
    this.needMoveHeight = this.needMoveHeight || 0
    this.needMoveHeight += height || this.itemHeight
    // let height = this.itemHeight
    let that = this
    if (!this.timer) {
      this.timer = setInterval(function() {
        if (that.needMoveHeight > that.options.speed) {
          that.moveDown(that.options.speed)
          that.needMoveHeight -= that.options.speed
        } else {
          that.moveDown(that.needMoveHeight)
          that.needMoveHeight = 0
          clearInterval(that.timer)
          that.timer = null
          that.clearItemsNotInView()
        }
        }, this.options.duration)
    }
  }

  SmoothSlide.prototype.clearItemsNotInView = function() {
  ...
  // 删除元素
  SmoothSlide.prototype.removeItem = function(item, force) {
    if (!item) {
      return
    }
    if (force) {
  ...
  SmoothSlide.prototype.calcHeight = function() {
    this.items = Array.prototype.slice.call(this.content.querySelectorAll(this.options.itemClass))
    const height = (this.items && this.items[0] && this.items[0].offsetHeight || 0)
    this.itemHeight = height + (height && this.options.heightDelta || 0)
